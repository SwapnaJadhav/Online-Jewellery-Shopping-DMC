...
const NoseRing = () => {
    const [noseRingProducts, setNoseRingProducts] = useState([]);

    useEffect(() => {
        fetchNoseRingProducts();
    }, []);

    const fetchNoseRingProducts = async () => {
        try {
            const response = await axios.get(`${config.server}/customer/sid/4`);
            setNoseRingProducts(response.data.data);
        } catch (error) {
        ...
        <>
            ...
            <Container>
                <Grid container spacing={3} style={{ marginTop: 10 }}>
                    {noseRingProducts.map((product, index) => (
